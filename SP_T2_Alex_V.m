...
t_dr = 0:0.02:D; %esantionarea semnalului original

x_dr = square((pi/12)*t_dr,0.5)/2+0.5; %Semnalul dreptunghiular
...
x_mod(t<=D)= x_dr; %modificam valorile nule cu valori din semnalul original
...
for k = -N:N %k este variabila dupa care se realizeaza suma
    ...
    x_t = x_dr; %x_t e semnalul realizat dupa formula SF data

    x_t = x_t .* exp(-1i*k*w0*t_dr); %inmultire intre doua matrice element cu element

    X(k+N+1) = 0; %initializare cu valoare nula

    for i = 1:length(t_dr)-1

        X(k+N+1) = X(k+N+1) + (t_dr(i+1)-t_dr(i)) * (x_t(i)+x_t(i+1))/2; %reconstructia folosind coeficientii
